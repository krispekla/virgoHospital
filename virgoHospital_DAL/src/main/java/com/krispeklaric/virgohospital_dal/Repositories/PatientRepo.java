...
public class PatientRepo extends BaseRepo implements IPatientRepo, Serializable {
    ...
    public Patient create(Patient patient) {
        ...
        try {
            ...
            if (doctor != null) {
                Patient oldPatientOfDoctor = doctor.getPatient();
                if (oldPatientOfDoctor != null) {
                    oldPatientOfDoctor.setDoctor(null);
                    oldPatientOfDoctor = em.merge(oldPatientOfDoctor);
                }
    ...
    public Patient edit(Patient patient) throws NonexistentEntityException, Exception {
        ...
        try {
            ...
            if (doctorOld != null && !doctorOld.equals(doctorNew)) {
                doctorOld.setPatient(null);
                doctorOld = em.merge(doctorOld);
            }
            if (doctorNew != null && !doctorNew.equals(doctorOld)) {
                Patient oldPatientOfDoctor = doctorNew.getPatient();
                if (oldPatientOfDoctor != null) {
                    oldPatientOfDoctor.setDoctor(null);
                    oldPatientOfDoctor = em.merge(oldPatientOfDoctor);
                }
                doctorNew.setPatient(patient);
